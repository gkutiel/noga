...
\begin{frame}{Risk-Aware Prediction}
  ...
  \begin{tikzpicture}[scale=1.0]
    ...
    % ---- Top-right: market bidding / asymmetric cost structure --------
    \panel{2}{(5,3)}{riskpurple}{Market Bidding}%
          {0.26*ifthenelse(\x<1,5*(1-\x),(\x-1))}
    ...
    \only<3->{
      ...
      \begin{scope}[shift={(2.5,0)}]
        ...
        \draw[very thick,netorange,line cap=round]
              plot[domain=0:2,samples=90,smooth]
              (\x,{0.75*exp(-((\x-0.7)*(\x-0.7))/0.15)+0.62*exp(-((\x-1.4)*(\x-1.4))/0.22)});
        \draw[riskpurple,thick,dashed] (1.5,0) -- (1.5,0.65);
        \fill[riskpurple] (1.5,0.65) circle (2.2pt);
        \node[netorange,anchor=north,font=\small\bfseries] at (1,-0.15) {Net Demand};
        \node[riskpurple,anchor=north,font=\small\bfseries] at (1.5,-0.55) {Risk-Aware Decision};
      \end{scope}
    }
  \end{tikzpicture}
\end{frame}
