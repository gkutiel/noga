...
20{,}770~km\textsuperscript{2} with a population of roughly 10~million people as of 2026.
The population geographic distribution is highly concentrated: the vast majority
...
\begin{figure}[H]
    ...
    features for demand forecasting.  Population density is per 1 square kilometre.}
    \label{fig:pop-density}
\end{figure}

The dataset consists of two complementary sources: electricity consumption data
obtained from the Israeli electricity system operator Noga~\parencite{noga},
and weather data sourced from the Israeli Meteorological Service~\parencite{ims}.
...
In Israel, where the percentage of households with air conditioning systems is currently 96.5\% (ranging from 89\% in the lower decile to 98.3\% in the upper decile) according to data from Israel's Central Bureau of Statistics, there is a strong correlation between electricity consumption levels and building cooling and heating loads resulting from changing weather conditions and building occupancy trends.
The weather data includes air temperature and relative humidity, whose combined effect controls building cooling loads, alongside wind speed measurements taken every three hours in the three major cities of Israel: Tel Aviv, Jerusalem, and Haifa.
...
Figure~\ref{fig:daily-demand} shows daily electricity consumption over the dataset period (1 March 2023 to 28 February 2026) alongside the daily average air temperature in Tel Aviv.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{plot/daily_demand_by_time.png}
    \caption{Daily electricity consumption (MWh) and observed air temperature over the dataset period
    (1 March 2023 to 28 February 2026).  Clear seasonal patterns are visible, with
...
Figure~\ref{fig:daily-demand-vs-temp} shows the relationship between daily electricity consumption and the daily average air temperature in Tel Aviv.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{plot/daily_demand_vs_avg_temp_Tel_Aviv.png}
    \caption{Daily electricity consumption vs.\ daily mean air temperature in
    Tel~Aviv.  The characteristic U-shaped relationship is visible, with
    high demand at both high air temperatures (cooling loads) and low air temperatures
    (heating loads).  The balance-point air temperature is approximately
    20\,$^\circ$C.}
...
\begin{table}[ht]
    ...
    \begin{tabular}{lrrrrr}
        ...
        Air Temp.\ Tel~Aviv ($^\circ$C)    & 22.1 & 4.8 &  9.6 & 22.5 & 30.3 \\
        Air Temp.\ Jerusalem ($^\circ$C)   & 18.9 & 6.4 &  2.3 & 19.8 & 36.5 \\
        Air Temp.\ Haifa ($^\circ$C)       & 20.3 & 5.1 &  4.4 & 20.8 & 31.3 \\
        \bottomrule
    \end{tabular}
...
\subsubsection{Forecast Error Distribution}%
...
\subsubsection{Comparison with the Noga Operational Forecast}
